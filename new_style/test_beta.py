...
# Generate underlying edge probabilities
names = net.names
P_alpha, P_beta = params['P_beta_params']
P = np.random.beta(P_alpha, P_beta, (params['N'],params['N']))
def edge_probabilities(network):
    N = network.N
    network_names = network.names

    inds = np.empty(N, dtype = np.int)
    for i in range(N):
        inds[i] = np.where(names == network_names[i])[0][0]

    return P[inds][:,inds]
data_model = IndependentBernoulli(edge_probabilities)
